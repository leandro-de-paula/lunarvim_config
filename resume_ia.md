...
- **Antigravity (Gemini 3.1 Pro High) - 2026-07-23:** Corrigiu o conflito de atalhos onde `<leader>dc` e `<leader>db` abriam o Debug Adapter Protocol (DAP) padrão do LunarVim. Redirecionou estes comandos via `which-key` para o Dadbod (banco de dados) e validou a necessidade de VPN para conexões assíncronas do Neovim.
